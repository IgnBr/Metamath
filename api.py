...
import json
from pyparsing import Literal, SkipTo, ParserElement, StringEnd, Optional, ZeroOrMore
import io

app = Flask(__name__)

@app.route('/metamath', methods=['POST'])
def echo():
    data = request.data.decode('utf-8')
    
    try:
        print('got request')
        treeData, nonComments, proofsStacks = doParsingAndValidation(data)
        title,description,contributors = parseAboutPage(data)
        response = jsonify({"data":treeData, "nonComments":nonComments,"proofsStacks":proofsStacks, "about":{"title":title,"description":description,"contributors":contributors}})
...
def doParsingAndValidation(metamath_file):
    mm = MM(None, None)
    ...
    return treeData, nonComments, proofsStacks
...
class MM:
        ...
        while tok and tok != '$}':
            if tok == '$c':
                ...
                if self.verify_proofs:
                    ...
                    proofsStacks.append({"proof":label, "stack":[], "floatingHypotheses":f_hyps,"essentialHypotheses":e_hyps, "stackHypotheses":[]})
    ...
    def treat_step(self,
        ...
        if steptype in ('$e', '$f'):
            ...
            proofsStacks[-1]["stackHypotheses"].append(([], []))
        elif steptype in ('$a', '$p'):
            dvs0, f_hyps0, e_hyps0, conclusion0 = stepdata
            proofsStacks[-1]["stackHypotheses"].append((f_hyps0, e_hyps0))
    ...
    def treat_normal_proof(self, proof: list[str]) -> list[Stmt]:
        ...
        proofsStacks[-1]["stack"].append(stacks)
        return stack
